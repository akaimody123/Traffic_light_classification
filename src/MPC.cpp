...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  size_t n_vars = 78;
  // TODO: Set the number of constraints(6 constraints at each time step)
 ...
 for (int i = 0; i < delta_start; i++) {
  ...
  //first two element are actuator values for nex step
  result.push_back(solution.x[delta_start]); 
  result.push_back(solution.x[a_start]);

  for (int i = 0; i < N-1; i++) {
    result.push_back(solution.x[x_start + i + 1]); //return predicted x value
  }

  for (int i = 0; i < N-1; i++) {
    result.push_back(solution.x[y_start + i + 1]); //return predicted y value
  }


   std::cout<<"steering "<<result[0]<<" throttle "<<result[1];
